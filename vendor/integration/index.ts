  ...
  return {
    ...
    hooks: {
      'astro:config:setup': async ({
        ...
        const buildLogger = logger.fork('sigmion');

        const virtualModuleId = 'sigmion:config';
        ...
        updateConfig({
          ...
          vite: {
            plugins: [
              {
                name: 'vite-plugin-sigmion-config',
                resolveId(id) {
        ...
        if (typeof _themeConfig === 'string') {
          ...
          buildLogger.info(`Sigmion \`${_themeConfig}\` has been loaded.`);
        } else {
          buildLogger.info(`Sigmion config has been loaded.`);
        }
      },
